...
def exec():
    csv_file = "static/files/preprocess.csv"
    headers = ["frame number","srcip","sport","dstip","dport","proto","service","tcp flag","frame length","TTL","window size","Duration","ignored"]

    if os.path.exists("static/files/out.csv"):
        os.remove("static/files/out.csv")

    with open("static/files/out.csv","w",newline='', encoding='utf-8') as g:
    ...
    def write(row):
        with open("static/files/out.csv","a",newline='', encoding='utf-8') as g:
            csv_writer = csv.writer(g)
